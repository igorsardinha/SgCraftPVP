...
public class Main extends JavaPlugin implements Listener
{
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        if (args[0].equalsIgnoreCase("on")) {
            ...
            if (this.safePlayers.contains(player.getUniqueId())) {
                ...
                player.sendMessage("  §f>> §aAgora você esta com o PVP §4ATIVADO!");
            }
            else {
        		...
        		player.sendMessage("§cVocê não tem permissao!");
        	}
        }
            ...
            if (!this.safePlayers.contains(player.getUniqueId())) {
                ...
                player.sendMessage("  §f>> §aAgora você esta com o PVP §4DESATIVADO!");
            }
            else {
    ...
    private void onCmd(final PlayerCommandPreprocessEvent e) {
		...
		if ((e.getMessage().toLowerCase().startsWith("/pvp") || e.getMessage().toLowerCase().startsWith("/sgcraftpvp:pvp"))) {
			if(player.getWorld().getName().equalsIgnoreCase("spawn") || player.getWorld().getName().equalsIgnoreCase("eventos") || player.getWorld().getName().equalsIgnoreCase("vip")){
			e.setCancelled(true);
    ...
    public void onPlayerHit(final EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof Player && e.getEntity() instanceof Player) {
            ...
            if (!this.safePlayers.contains(damager.getUniqueId()) && this.safePlayers.contains(victim.getUniqueId())) {
                ...
                damager.sendMessage("  §f>> §cVocê esta com o PVP §4DESATIVADO§c!");
                e.setCancelled(true);
            }
            else if (this.safePlayers.contains(damager.getUniqueId()) && this.safePlayers.contains(victim.getUniqueId())) {
                damager.sendMessage("  §f>> §cAmbos os jogadores estão com PVP &4DESATIVADO!");
                e.setCancelled(true);
            }
        }
    }
